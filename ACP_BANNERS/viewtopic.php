...
$sql = "
SELECT
    f3.forum_id as region_id,
    f4.forum_id as category_id
FROM
    phpbb_forums f1
LEFT JOIN
    phpbb_forums f2
ON
    f1.parent_id = f2.forum_id
LEFT JOIN
    phpbb_forums f3
ON
    f2.parent_id = f3.forum_id
LEFT JOIN
    phpbb_forums f4
ON
    f3.parent_id = f4.forum_id
WHERE
    f1.forum_id = ".$forum_id;

$result = $db->sql_query($sql);
$lines = $db->sql_fetchrow($result);

foreach ( $lines as $line ):
	echo $line['region_id'].'<br />';
	echo $line['category_id'].'<br />';
endforeach;
